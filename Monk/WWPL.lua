...
A[3] = function(icon)
	...
	local function BasicDamageRotation(unit)
		...
		if A.WhirlingDragonPunch:IsReady(player) and inMelee then
		...
		if
			A.SpinningCraneKick:IsReady(player)
		...
		if
			...
			and inMelee
			and unitCount >= 2
			and A.FistsOfFury:GetCooldown() > 1
		then
			return A.SpinningCraneKick:Show(icon)
		end
